...
cd $UDIRNAME
./configure --prefix=$PREFIX/$DIRNAME-$TOOLCHAIN
make
make install
cd ..

export LD_RUN_PATH=$PREFIX/$DIRNAME-$TOOLCHAIN/lib:$LD_LIBRARY_PATH
export LIBRARY_PATH=$PREFIX/$DIRNAME-$TOOLCHAIN/lib:$LIBRARY_PATH
export CPATH=$PREFIX/$DIRNAME-$TOOLCHAIN/include:$CPATH
cd $DIRNAME
./configure --prefix=$PREFIX/$DIRNAME-$TOOLCHAIN
make
make install
